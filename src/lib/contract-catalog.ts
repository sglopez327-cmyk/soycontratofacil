...
export const contractCatalog: CatalogCategory[] = [
  {
    ...
    contracts: [
      ...
      },
      {
        ...
        description: "Arrendamiento de plaza de garaje",
      },
    ],
  },
  {
    ...
    contracts: [
      ...
      {
        id: "compraventa-garaje",
        slug: "compraventa-garaje",
        title: "Plaza de garaje",
        description: "Compraventa de plaza de garaje entre particulares",
      },
      {
        id: "garaje-trastero",
...
export function resolveContractSlug(slug: string): string {
  return slug;
}
...
export function findCatalogContract(slug: string) {
  const resolved = resolveContractSlug(slug);
  for (const category of contractCatalog) {
    const contract = category.contracts.find((item) => item.slug === resolved);
    if (contract) {
