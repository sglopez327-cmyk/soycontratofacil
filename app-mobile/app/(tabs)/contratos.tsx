    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: colors.background }]} edges={["left", "right"]}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <Text style={[styles.intro, { color: colors.textMuted }]}>
          Elige un contrato y complétalo paso a paso con el mismo formulario y validaciones que la web.
        </Text>
        ...
        {contractCategories.map((category) => (
          <View key={category.id} style={styles.category}>
            ...
            {category.contracts.map((contract) => (
              ...
              >
                <Text style={[styles.cardTitle, { color: colors.text }]}>{contract.title}</Text>
                <Text style={[styles.cardDescription, { color: colors.textMuted }]}>{contract.description}</Text>
              </Pressable>
            ))}
          </View>
        ))}
