      ...
      {field.type === "checkbox" ? (
        ...
        <View style={[styles.pickerWrapper, { backgroundColor: colors.card, borderColor: error ? "#f87171" : colors.border }, disabled && styles.inputDisabled]}>
          <Picker
            selectedValue={value || ""}
            enabled={!disabled}
            onValueChange={(itemValue) => { if (itemValue !== "") onChange(String(itemValue)); }}
        ...
        <View>
          <Pressable onPress={() => setShowDatePicker(true)}>
            <TextInput value={value} editable={false} pointerEvents="none" placeholder="AAAA-MM-DD" placeholderTextColor={colors.textMuted} style={inputStyle} />
          </Pressable>
          {showDatePicker ? (
            <DateTimePicker
          ...
          {Platform.OS === "ios" && showDatePicker ? (
            <Text style={[styles.dateDone, { color: Colors.brand.blue }]} onPress={() => setShowDatePicker(false)}>Listo</Text>
          ) : null}
        </View>
      ) : (
        <TextInput
          ...
          placeholderTextColor={colors.textMuted}
          editable={!disabled}
          keyboardType={field.type === "number" ? "numeric" : field.type === "email" ? "email-address" : field.type === "currency" ? "decimal-pad" : "default"}
...
const styles = StyleSheet.create({
  ...
  checkboxRow: { flexDirection: "row", alignItems: "center", gap: 12, padding: 14, borderRadius: 12, borderWidth: 1 },
