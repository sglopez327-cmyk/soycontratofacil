...
export function ContractWizard({ config, contractTitle }: ContractWizardProps) {
  ...
  async function handleGenerateOnWeb() {
    if (isOpeningWeb) return;
    setIsOpeningWeb(true);
    try {
      await WebBrowser.openBrowserAsync(getContractWebUrl(config.slug));
    } finally {
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.stepsRow}>
        {steps.map((step, index) => {
          ...
          return (
            ...
            >
              <Text style={[styles.stepChipText, { color: isActive ? "#fff" : isDone ? Colors.brand.emerald : colors.textMuted }]}>
                {index + 1}. {step.title}
              </Text>
            </Pressable>
          );
        ...
        {isComplete ? (
          <View style={styles.content}>
            ...
            <Text style={[styles.description, { color: colors.textMuted }]}>
              Revisa el resumen de tu contrato de {contractTitle}. La descarga de PDF en el dispositivo llegará en una próxima versión.
            </Text>
            <View style={[styles.summaryList, { borderColor: colors.border }]}>
              {allFields.filter((f) => values[f.id]?.trim()).map((field) => (
                <View key={field.id} style={[styles.summaryRow, { borderColor: colors.border }]}>
                  ...
                  <Text style={[styles.summaryValue, { color: colors.text }]}>{formatContractFieldValue(field, values[field.id])}</Text>
                </View>
              ))}
            </View>
            <View style={styles.actions}>
              <Pressable style={[styles.secondaryButton, { borderColor: colors.border }]} onPress={() => setIsComplete(false)}>
                <Text style={[styles.secondaryButtonText, { color: colors.text }]}>Editar datos</Text>
              </Pressable>
              <Pressable style={styles.primaryButton} disabled={isOpeningWeb} onPress={handleGenerateOnWeb}>
                {isOpeningWeb ? <ActivityIndicator color="#fff" /> : <Text style={styles.primaryButtonText}>Generar PDF en web</Text>}
              </Pressable>
            </View>
          </View>
        ) : (
          <View style={styles.content}>
            <Text style={[styles.stepLabel, { color: Colors.brand.blue }]}>Paso {currentStepIndex + 1} de {steps.length}</Text>
            <Text style={[styles.heading, { color: colors.text }]}>{currentStep.title}</Text>
            {currentStep.description ? <Text style={[styles.description, { color: colors.textMuted }]}>{currentStep.description}</Text> : null}
            <View style={styles.fields}>
              {currentStep.partyForm ? <PartesForm variant={currentStep.partyForm} values={values} errors={errors} onChange={updateValue} /> : null}
              {currentStep.fields.map((field) => (
                <FormField key={field.id} field={field} value={values[field.id] ?? ""} error={errors[field.id]} onChange={(v) => updateValue(field.id, v)} />
              ))}
            </View>
            <View style={styles.actions}>
              <Pressable style={[styles.secondaryButton, { borderColor: colors.border, opacity: isFirstStep ? 0.5 : 1 }]} disabled={isFirstStep} onPress={() => { setIsComplete(false); setCurrentStepIndex((i) => Math.max(0, i - 1)); setErrors({}); }}>
                <Text style={[styles.secondaryButtonText, { color: colors.text }]}>Anterior</Text>
              </Pressable>
              <Pressable style={styles.primaryButton} onPress={handleNext}>
                <Text style={styles.primaryButtonText}>{isLastStep ? "Revisar y finalizar" : "Siguiente"}</Text>
              </Pressable>
            </View>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  primaryButton: { flex: 1, backgroundColor: Colors.brand.blue, borderRadius: 12, paddingVertical: 14, alignItems: "center", minHeight: 48, justifyContent: "center" },
  primaryButtonText: { color: "#fff", fontSize: 15, fontWeight: "600", textAlign: "center" },
  secondaryButton: { flex: 1, borderWidth: 1, borderRadius: 12, paddingVertical: 14, alignItems: "center", minHeight: 48, justifyContent: "center" },
